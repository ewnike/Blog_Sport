...
class OddsNHL(models.Model):
    ...
    schedule = models.ForeignKey(ScheduleNHL, related_name='odds', null = True, blank = False,  on_delete = models.CASCADE)
    last_updated=models.DateTimeField(blank=False)

class GameOdds(models.Model):
    entity_id = models.ForeignKey(OddsNHL, related_name = "entity_id", on_delete=models.CASCADE, null=True)
    game_id = models.ForeignKey(ScheduleNHL, related_name = 'game_id', null= True, blank=True,  on_delete = models.CASCADE)
